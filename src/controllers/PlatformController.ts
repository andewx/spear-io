...
export class PlatformController {
  ...
  async listAll(req: Request, res: Response): Promise<void> {
    try {
      ...
      const response: TAPIResponse<typeof platforms> = {
        success: true,
        data: platforms,
      };
      res.json(response);
  ...
  async getById(req: Request, res: Response): Promise<void> {
    try {
      ...
      const response: TAPIResponse<typeof platform> = {
        success: true,
        data: platform,
      };
      res.json(response);
  ...
  async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { type } = req.body as { type: 'sam' | 'fighter'; data: ISAMSystem | IFighterPlatform };
      ...
      if (type === 'sam'){
        ...
        data = fighterData as IFighterPlatform;
      }
      ...
      if (type === 'sam') {
        await storage.saveSAMPlatform(data as ISAMSystem);
      } else {
        await storage.saveFighterPlatform(data as IFighterPlatform);
      }

      const response: TAPIResponse<typeof data> = {
        success: true,
        data,
      };
